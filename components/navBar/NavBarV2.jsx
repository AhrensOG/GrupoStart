...
const NavBarV2 = ({ home = true, servicesDropdown = true }) => {
  const [nav, setNav] = useState(false);
  const [isOpen, setIsOpen] = useState(false);
  const [burgerColor, setBurgerColor] = useState('white')

  return (
    <div className="absolute top-0 w-full h-[80px] bg-transparent px-20 mt-6">
      {/* NAVBAR */}
      ...
      <div
        ...
        className={`${
          !nav && "hidden"
        } sm:hidden bg-black/90 min-h-screen w-full fixed top-0 left-0 right-0 backdrop-blur-sm z-10`}
      ></div>
